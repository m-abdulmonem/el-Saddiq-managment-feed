...
class InvoicesServices extends ClientBill
{
    ...
    {
        $invoices = mapArray($this->where("client_id", $id)->get(),function ($c,$v){return $c->push($v->invoices()->latest()->get());});

        return mapArray($invoices,function ($c,$invoices){
            foreach ($invoices as $invoice) $c->push($invoice);
            return $c;
        });
    }
    ...
    public function createProduct($request,$update= false)
    {
        $callback = function ($c,$k,$id) use ($request,$update){
            ...
            return $c->put($id,[
                ...
                'purchase_price' => ProductStock::purchasedProduct($id,$request->stock_id[$k])->latest()->first()->piece_price,
    ...
    {
        $data = [
            'remaining_amount' => (!$request->debt) ? ($request->total_price - $request->postpaid) : 0,
            'paid' => ($request->debt == "postpaid" ? $request->postpaid : $request->total_price),
            'type' => $type,
    ...
    {
        $url = route("ajax.client.print.invoice",$this->id);
