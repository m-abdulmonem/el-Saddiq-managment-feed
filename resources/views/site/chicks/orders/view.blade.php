                ...
                <div class="card-body d-flex main-info">
                    <div class="col-6">
                        <ul class="list-ar-numeric">
                            <li><span class="title odd">@lang("$trans.quantity")</span> {{ $order->booking()->sum("quantity") }} @lang("chicks/chicks.chick")</li>
                            <li><span class="title even">@lang("$trans.price")</span> {{ currency($order->price) }}</li>
                            <li><span class="title odd">@lang("$trans.chick_price")</span> {{ currency($order->chick_price) }}</li>
                            <li><span class="title even">@lang("chicks/chicks.chick")</span> {{$order->chick->name}}</li>
                            <li><span class="title even">@lang("$trans.sold_quantity")</span> {{$order->soldQuantity()}}</li>
                        </ul>
                    </div>
                    <div class="col-6 float-right">
                        <ul class="list-ar-numeric">
                            <li><span class="title odd">@lang("balances.gain")</span> {{ currency($order->debt()->sum("gain")) }}</li>
                            <li><span class="title even">@lang("balances.loss")</span> {{ currency($order->debt()->sum("loss")) }}</li>
                            <li><span class="title odd">@lang("$trans.status")</span> @lang("$trans._option_$order->is_came")</li>
                            <li><span class="title even">@lang("$trans.arrived_at")</span> {{ $order->arriveAtFormat() }}</li>
                            <li><span class="title even">@lang("$trans.unsold_quantity")</span> {{ $order->unsoldQuantity() }}</li>
                        </ul>
                    </div>
                </div>
                <!-- /.card-body -->
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-6">
                            <label for="transactionDate">@lang("balances.select_date")</label>
                ...
                <div class="card-footer d-flex ">
                    <div class="col-4  d-flex">
                        <h4>@lang("balances.paid") : <span class="info-color ml-2">{{ currency($order->balances()->sum("paid")) }}</span></h4>
                    </div>
                    <div class="col-4 d-flex">
                        <h4 >@lang("balances.creditor") : <span class="info-color ml-2">{{ currency($order->creditorSupplier()) }}</span></h4>
                    </div>
                    <div class="col-4 d-flex">
                        <h4>@lang("balances.debtor") : <span class="info-color ml-2">{{ currency($order->debtorSupplier()) }}</span></h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-6">
                            <label for="clientDate">@lang("balances.select_date")</label>
                            <div id="clientDate" style="background: #fff; cursor: pointer; padding: 5px 10px; border: 1px solid #ccc; width: 100%">
                                <i class="fa fa-calendar"></i>&nbsp;
                                <span></span> <i class="fa fa-caret-down"></i>
                            </div>
                        </div>
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <select id="select_client_transaction_type" class="form-control">
                                    {!! select_options(['','prev_balance','catch','payment','sale','buy'],null,null,"balances") !!}
                                </select>
                ...
                <div class="card-footer d-flex ">
                    <div class="col-4  d-flex">
                        <h4>@lang("balances.paid") : <span class="info-color ml-2">{{ currency($order->clientPaid()->sum()) }}</span></h4>
                    </div>
                    <div class="col-4 d-flex">
                        <h4>@lang("balances.debtor") : <span class="info-color ml-2">{{ currency($order->debtorClient()) }}</span></h4>
                    </div>
                    <div class="col-4 d-flex">
                        <h4>@lang("balances.creditor") : <span class="info-color ml-2">{{ currency($order->creditorClient()) }}</span></h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            $(function () {
                ...
                transactionDate.on('apply.daterangepicker', function(ev, picker) {
                    ...
                    $("#transactions").DataTable().destroy();
                    transactions($("#select_transaction_type").val(),start,end);
                });
            ...
            function transactions(type = null,start = null, end =null) {
                return  $("#transactions").table({
                    ...
                    url: "{{ route("ajax.chicks.orders.balances.supplier",$order->id) }}",
            ...
            function clientTransactions(type = null,start = null, end =null) {
                return  $("#clientTransactions").table({
                    ...
                    url: "{{ route("ajax.chicks.orders.balances.client",$order->id) }}",
                    data: {type: type,startDate: start,end: end},
                    notColumns: [
                        '#',
                        'actions'
